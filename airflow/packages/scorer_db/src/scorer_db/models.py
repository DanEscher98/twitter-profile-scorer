...
class UserProfile(SQLModel, table=True):
    ...
    # Relationships
    scores: list[ProfileScore] = Relationship(back_populates="profile")
    stats: UserStats | None = Relationship(back_populates="profile")
    keywords: list[UserKeyword] = Relationship(back_populates="profile")
    to_score_entry: ProfileToScore | None = Relationship(back_populates="profile")
...
class ProfileScore(SQLModel, table=True):
    ...
    scored_at: datetime = Field(default_factory=datetime.utcnow, index=True)
    scored_by: str = Field(max_length=100, index=True)
    audience: str | None = Field(default=None, max_length=100, index=True)

    # Relationship
    profile: UserProfile | None = Relationship(back_populates="scores")
...
class ApiSearchUsage(SQLModel, table=True):
    ...
    query_at: datetime = Field(default_factory=datetime.utcnow)
    platform: Platform = Field(default=Platform.TWITTER, max_length=20)

    # Relationships
    user_keywords: list[UserKeyword] = Relationship(back_populates="search")
...
class ProfileToScore(SQLModel, table=True):
    ...
    # Relationship
    profile: UserProfile | None = Relationship(back_populates="to_score_entry")
...
class UserKeyword(SQLModel, table=True):
    ...
    # Relationships
    profile: UserProfile | None = Relationship(back_populates="keywords")
    search: ApiSearchUsage | None = Relationship(back_populates="user_keywords")
...
class UserStats(SQLModel, table=True):
    ...
    # Relationship
    profile: UserProfile | None = Relationship(back_populates="stats")
